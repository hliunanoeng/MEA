...
def slopePlot(l_set,dic):
    ...
    for i in range(l-1):
        ...
        ax1.text(i+1.5,min(l_a+l_b)+((max(l_a+l_b)-min(l_a+l_b))/2),"p="+str(WilcoxonTest(l_a,l_b)[1])[:6])
    ...
    ax1.set_xticklabels(xticklabels,size=6)
    ax1.set_xlabel(dic['xlabel'],size=8)
    ax1.set_ybound(ybounds[0],ybounds[-1])
    ax1.set_yticks(ybounds)
    ax1.set_yticklabels(ybounds,size=6)
    ax1.set_ylabel(dic['ylabel'],size=8)
...
slopePlot([[0,0,0,0,0,0,61,10,10,0,28,50],[60,65,80,98,95,92,90,99,99,96,90,86]],{'label1':'SDFO','label2':'LDMO','xlabel':'Two Options for High Seal Resistance','ylabel':'Activity (%)','title':"SDFO vs LDMO"})
